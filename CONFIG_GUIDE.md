...
正式页的“找到我”和后续内容列共用 `--home-column-width`。首屏序列用参与普通布局的内容盒 spacer 提供 `--intro-travel`，因此 sticky 场景不会在动画完成前被父级提前推出视口；不要把它改回只有 `padding-bottom` 的布局。`src/intro-scroll.js` 始终用解析后的 `--intro-travel` 驱动 0–1 动画进度，“找到我”约在 45% 开始淡入、75% 完全显示。增加平台卡或使用很矮的浏览器窗口时，超出视口的部分只增加动画完成后的自然滚动空间，不会拉长动画或覆盖下一板块。

Sites 版 React 首页先以本文件所述的静态 `config.js` 保底值启动，再独立请求 `/api/public/config`。有效远端配置可以在页面已经出现后安全替换 Orbit 与平台卡；请求失败或响应缓慢不会阻塞首屏初始化。
