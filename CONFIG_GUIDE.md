...
正式页的“找到我”和后续内容列共用 `--home-column-width`。首屏场景高度取视口高度与平台内容实际高度的较大值，`src/intro-scroll.js` 则用首屏容器高度减去场景高度得到纯动画滚动距离；所以增加平台卡或使用很矮的浏览器窗口时，只会增加动画完成后的自然滚动空间，不会拉长动画或覆盖下一板块。
