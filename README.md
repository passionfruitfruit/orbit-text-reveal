...
完整 Sites/Vinext 工具链使用 Node 22 LTS：`.nvmrc` 固定本地版本为 `22.17.0`，`package.json` 将兼容范围限制为 `>=22.17 <23`。本机 Node 26 会在当前 Vinext/Vite 模块加载阶段异常变慢；运行开发服务器或生产构建前请先切换到 Node 22。
...
“找到我”平台卡与后续内容统一使用 `--home-column-width`：最大宽度为 `760px`，窄屏保留左右各 `16px`。首屏序列通过真实的内容盒 spacer 提供 `--intro-travel`，保证 sticky 场景在整个动画行程内停留在视口；不能只用底部 padding 伪造 `scrollHeight`。平台从首屏进度约 `45%` 开始出现、约 `75%` 完全可见。短视口下平台区会按实际内容自然增高，超出视口的部分只形成动画完成后的自然滚动空间，不会拉长动画或覆盖“最近在做”。

React 首页会先使用 `config.js` 的静态保底内容启动。`/api/public/config` 独立加载，成功后通过页面结果对象安全更新 Orbit 和平台卡；接口失败或响应较慢不会让首屏一直保持隐藏。
